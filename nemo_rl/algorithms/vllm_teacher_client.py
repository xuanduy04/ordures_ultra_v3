...
def probe_teacher_serve(url: str, models: set[str], timeout_s: float = 30.0) -> None:
    """GET ``{url}/models`` and verify every expected ``model`` is served.
...
class VLLMTeacherLogprobClient:
        ...
        try:
            ...
            async def _create_session() -> None:
                connector = aiohttp.TCPConnector(
                    limit=_MAX_CONCURRENT_REQUESTS, keepalive_timeout=60.0
                )
    ...
    async def _post_with_retries(
        ...
        """
        completions_url = f"{teacher['url'].rstrip('/')}/completions"
        attempt = 0
