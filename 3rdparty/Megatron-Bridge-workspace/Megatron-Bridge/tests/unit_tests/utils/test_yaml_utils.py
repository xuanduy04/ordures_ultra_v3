...
from megatron.bridge.utils.yaml_utils import (
    _enum_representer,
...
class TestDumpDataclassToYaml:
    """Test the dump_dataclass_to_yaml function."""

    def test_dump_to_string(self):
        """Test dumping an object to a YAML string."""
        test_obj = TestDataclass(name="test", value=42)
        result = dump_dataclass_to_yaml(test_obj)
    ...
    def test_dump_to_file(self):
        """Test dumping an object to a YAML file."""
        test_obj = TestDataclass(name="test", value=42)
        ...
        try:
            result = dump_dataclass_to_yaml(test_obj, temp_filename)
            assert result is None  # Should return None when writing to file

            # Verify file contents
            with open(temp_filename, "r") as f:
                content = f.read()
            assert "_target_" in content
            assert "_call_" in content
            assert "TestDataclass" in content
        finally:
            os.unlink(temp_filename)

    def test_dump_complex_object_with_function(self):
        """Test dumping an object that contains a function."""
        test_obj = {"func": test_function, "data": "test"}

        result = dump_dataclass_to_yaml(test_obj)
        assert isinstance(result, str)
        assert "_target_:" in result
        assert "test_yaml_utils.test_function" in result
...
class TestIntegration:
    ...
    def test_complex_object_serialization(self):
        ...
        result = dump_dataclass_to_yaml(complex_obj)
        assert isinstance(result, str)
    ...
    def test_roundtrip_with_simple_objects(self):
        ...
        simple_obj = {"string": "test", "number": 42, "list": [1, 2, 3], "dict": {"nested": "value"}}

        yaml_str = dump_dataclass_to_yaml(simple_obj)
