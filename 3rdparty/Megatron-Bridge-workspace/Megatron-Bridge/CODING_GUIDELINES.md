...
1. The code developed for Megatron-Bridge should conform to Python 3.10+.
...
3. Third-party imports (including `megatron.core`, `torch`, `transformers`)
4. First-party imports (`megatron.bridge.*`)
...
1. Create a new directory under `src/megatron/bridge/models/<model_name>/`
2. Implement the parameter mapping in `param_mapping.py`
3. Implement the model bridge in `model_bridge.py`
4. Register the model in the appropriate registry

### Weight Conversion

1. Always validate tensor shapes before copying weights.
2. Handle tensor parallel and pipeline parallel distribution correctly.
3. Use `print_rank_0` for logging to avoid duplicate output across ranks.
...
1. Model configuration defaults
