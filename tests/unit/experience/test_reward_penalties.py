...
# Extract and exec just the function to avoid the heavy dependency chain.
try:
    ...
    _match = re.search(
        r'^(def apply_reward_penalties\(.+?)(?=\ndef )', _src, re.DOTALL | re.MULTILINE
    )
...
def _msg(role, token_ids):
    return {"role": role, "token_ids": torch.tensor(token_ids)}
...
class TestPenalizeMultiEndThink:
    ...
    def test_multi_turn_history_think_tokens_valid(self):
        ...
        result = _make_result(reward=1.0, message_log=[
            _msg("user", [100, 12, 13, 12, 13, 12, 13, 12, 13, 12]),  # 5 open, 4 close
    ...
    def test_piecemeal_double_think_close_in_generation_penalized(self):
        ...
        result = _make_result(
            reward=1.0,
            output_items=[_message_item("answer", generation_str="</think> text </think>")],
            message_log=[
                _msg("user", [100, 12]),
                _msg("assistant", [300, 13, 400]),  # token IDs are fine
            ],
    ...
    def test_single_think_close_in_generation_str_ok(self):
        ...
        result = _make_result(
            reward=1.0,
            output_items=[_message_item("answer", generation_str="thinking </think> answer")],
            message_log=[
                _msg("user", [100, 12]),
                _msg("assistant", [300, 13, 400]),
            ],
...
class TestCrossCutting:
    ...
    def test_batch_of_results_mixed(self):
        ...
        cfg = {"penalize_eos_token": True}
        r1 = _make_result(reward=1.0, message_log=[
            _msg("user", [100]),
            _msg("assistant", [300, 400]),
        ])
        r2 = _make_result(reward=1.0, message_log=[
            _msg("user", [100]),
